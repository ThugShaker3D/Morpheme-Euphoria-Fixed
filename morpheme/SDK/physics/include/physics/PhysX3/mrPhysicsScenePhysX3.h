...
protected:
  physx::PxQueryHitType::Enum preFilter(const physx::PxFilterData& filterData, const physx::PxShape* shape, const physx::PxRigidActor* actor, physx::PxHitFlags& filterFlags) NM_OVERRIDE;
  physx::PxQueryHitType::Enum postFilter(const physx::PxFilterData& filterData, const physx::PxSceneQueryHit& hit) NM_OVERRIDE;
...
{ 
  if (actor.is<physx::PxRigidBody>())
    actorToWorld = nmPxTransformToNmMatrix34(actor.is<physx::PxRigidBody>()->getGlobalPose());
} 
...
{ 
  if (actor.is<physx::PxRigidBody>())
      actor.is<physx::PxRigidBody>()->setGlobalPose(nmMatrix34ToPxTransform(newActorToWorld), true);
}
...
{
  if (actor.is<physx::PxRigidBody>())
      actor.is<physx::PxRigidBody>()->addForce(nmVector3ToPxVec3(velChange), physx::PxForceMode::eVELOCITY_CHANGE);
}
...
{
  if (actor.is<physx::PxRigidBody>())
      actor.is<physx::PxRigidBody>()->addTorque(nmVector3ToPxVec3(angularVelChange), physx::PxForceMode::eVELOCITY_CHANGE);
}
...
{
  if (actor.is<physx::PxRigidBody>())
      actor.is<physx::PxRigidBody>()->addTorque(nmVector3ToPxVec3(angularAccel), physx::PxForceMode::eACCELERATION);
}
...
{
  if (actor.is<physx::PxRigidBody>())
    actor.is<physx::PxRigidBody>()->addForce(nmVector3ToPxVec3(accel), physx::PxForceMode::eACCELERATION);
}
...
{
  if (actor.is<physx::PxRigidBody>())
    actor.is<physx::PxRigidBody>()->addTorque(nmVector3ToPxVec3(torqueImpulse), physx::PxForceMode::eIMPULSE);
}
...
{
  if (actor->is<physx::PxRigidBody>())
    return actor->is<physx::PxRigidBody>()->getMass();
  else
    return 0.0f;
}
