...
public:
  ...
  {
      materials.clear();
      materials.resize(materialCount_);
      memcpy(materials.data(), materials_, sizeof(physx::PxMaterial*) * materialCount_);
  }
...
PX_INLINE PxActorDesc::PxActorDesc(physx::PxActorType::Enum t) : type(t)
{
  ...
  clientBehaviorFlags  = physx::PxClientBehaviorFlags();
  userData      = NULL;
  name        = NULL;
}
...
{
public:
  std::vector<const PxShapeDesc*> shapes;
  PX_INLINE void setShapes(const PxShapeDesc*const* shapes_, physx::PxU32 shapeCount_)
  {
    shapes.clear();
    shapes.resize(shapeCount_);
    memcpy(shapes.data(), shapes_, sizeof(PxShapeDesc*) * shapeCount_);
  }
  PX_INLINE void setSingleShape(const PxShapeDesc& shape)
  {
    shapes.clear();
    shapes.push_back(new PxShapeDesc(shape));
  }
protected:
...
{
  massLocalPose      = physx::PxTransform(physx::PxIdentity);
  mass               = 1.0f;
...
public:
  ...
  physx::PxRigidBodyFlags    rigidDynamicFlags;
  PX_INLINE void setToDefault(const physx::PxTolerancesScale& scale);
...
: PxRigidBodyDesc(physx::PxActorType::eRIGID_DYNAMIC)
{
  ...
  rigidDynamicFlags    = physx::PxRigidBodyFlags();
}
...
{
  parentPose      = physx::PxTransform(physx::PxIdentity);
  childPose      = physx::PxTransform(physx::PxIdentity);

  targetPosition    = physx::PxQuat(physx::PxIdentity);
...
template<typename DescType, typename ActorType>
void createShapes_Static(const DescType& desc, ActorType& actor)
{
    for (physx::PxU32 iShape = 0; iShape != desc.shapes.size(); ++iShape)
    {
        const PxShapeDesc& shapeDesc = *desc.shapes[iShape];
...
PX_INLINE physx::PxRigidDynamic* PxCreateRigidDynamic(const PxRigidDynamicDesc& desc)
{
  ...
  result->setRigidBodyFlags(desc.rigidDynamicFlags);
  ...
  result->setClientBehaviorFlags((physx::PxActorClientBehaviorFlags)desc.clientBehaviorFlags);
...
PX_INLINE physx::PxRigidStatic* PxCreateRigidStatic(const PxRigidStaticDesc& desc)
{
  ...
  // PxRigidActorDesc
  createShapes_Static(desc, *result);
