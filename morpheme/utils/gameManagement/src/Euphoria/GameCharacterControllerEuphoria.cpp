...
physx::PxControllerFilters g_physXControllerCollisionFilter(0, &g_morphemePhysX3QueryFilterCallback, 0);
//-----------
...
{  
  ...
  if (physicsRig)
  {
    ...
    physicsRig->setCharacterControllerActor(m_physXController->getActor()->is<physx::PxRigidDynamic>());
  }
  ...
  physx::PxMaterial*      controllerMaterial)
{
  ...
  m_physXController = m_physXControllerManager->createController(*desc);
  NMP_ASSERT(m_physXController);
...
void CharacterControllerPhysX3::resolveKinematicActorPenetration(MR::PhysicsRigPhysX3* physicsRig)
{
  ...
  if (m_physXRigidDynamicActor)
  {
    ...
    // into the callback.
    physx::PxSceneQueryFilterData sceneQueryFilterData(queryFlags);
    ...
    physx::PxOverlapHit hits[32];
    physx::PxOverlapBuffer hit(hits, 32);
    ...
    if (physXScene->overlap(
                      ...
                      &g_morphemePhysX3QueryFilterCallback) && hit.hasBlock)
    {
  ...
  physx::PxU32 controllerCollisionFlags)
{
  ...
  m_onGround = (controllerCollisionFlags & physx::PxControllerCollisionFlag::eCOLLISION_DOWN) != 0 ;
