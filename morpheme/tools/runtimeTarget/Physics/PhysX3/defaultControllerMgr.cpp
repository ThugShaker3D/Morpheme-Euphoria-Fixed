...
{
  physx::PxControllerBehaviorFlags getBehaviorFlags(const physx::PxShape& NMP_UNUSED(shape), const physx::PxActor& NMP_UNUSED(actor)) NM_OVERRIDE {return physx::PxControllerBehaviorFlags();}
  physx::PxControllerBehaviorFlags getBehaviorFlags(const physx::PxController& NMP_UNUSED(controller)) NM_OVERRIDE {return physx::PxControllerBehaviorFlags();}
  physx::PxControllerBehaviorFlags getBehaviorFlags(const physx::PxObstacle& NMP_UNUSED(obstacle)) NM_OVERRIDE {return physx::PxControllerBehaviorFlags();}
};
  ...
  const NMP::Quat&    characterStartRotation)
{
  ...
  desc.reportCallback = ccRecord->m_hitReport;
  ...
  for (Records::iterator it = m_ccRecords.begin(); it != m_ccRecords.end(); ++it)
  {
    ...
    { 
      ...
      if (ccRecord->m_pxRigidDynamicActor)
      {
        ...
        // that.
        physx::PxQueryFlags queryFilterFlags = 
        ...
        physx::PxOverlapHit buffers[32];
        physx::PxOverlapBuffer overlapBuffer(buffers, 32);

        if (physXScene->overlap(
          geom,
          globalPose,
          overlapBuffer,
