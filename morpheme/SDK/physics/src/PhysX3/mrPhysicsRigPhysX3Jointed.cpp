...
void PhysicsRigPhysX3Jointed::PartPhysX3Jointed::generateCachedValues()
{
  ...
  m_cache.bodyFlags = (uint16_t)getRigidDynamic()->getRigidBodyFlags();
...
{
  if ((m_cache.bodyFlags & physx::PxRigidBodyFlag::eKINEMATIC) != 0)
  {
...
void PhysicsRigPhysX3Jointed::PartPhysX3Jointed::makeKinematic(bool kinematic, float NMP_UNUSED(massMultiplier), bool NMP_UNUSED(enableConstraint))
{
  ...
  if (kinematic)
  {
    m_cache.bodyFlags |= physx::PxRigidBodyFlag::eKINEMATIC;
  }
  else
  {
    m_cache.bodyFlags &= ~(uint16_t)physx::PxRigidBodyFlag::eKINEMATIC;
  }
}
...
{
  return (getRigidDynamic()->getRigidBodyFlags() & physx::PxRigidBodyFlag::eKINEMATIC);
}
...
{
  if (slerpStrength != m_cache.slerpDrive.stiffness)
  {
    m_dirtyFlags |= kDirty_SlerpDrive;
    m_cache.slerpDrive.stiffness = slerpStrength;
  }
  if (swingStrength != m_cache.swingDrive.stiffness)
  {
    m_dirtyFlags |= kDirty_SwingDrive;
    m_cache.swingDrive.stiffness = swingStrength;
  }
  if (twistStrength != m_cache.twistDrive.stiffness)
  {
    m_dirtyFlags |= kDirty_TwistDrive;
    m_cache.twistDrive.stiffness = twistStrength;
  }
}
