  ...
  float physicsToleranceScale)
{
  ...
  m_physXFoundation = PxCreateFoundation(PX_FOUNDATION_VERSION, m_physXAllocator, m_physXErrorStream);
  NMP_ASSERT(m_physXFoundation);

  // Create a new PhysX profile zone manager.
  m_physXVisualDebugger = physx::PxCreatePvd(*m_physXFoundation);
  NMP_ASSERT(m_physXVisualDebugger);

  // Creates an instance of the PhysX SDK.
  m_physXSDK = PxCreatePhysics(PX_PHYSICS_VERSION, *m_physXFoundation, scale, true, m_physXVisualDebugger);
  ...
  PxInitExtensions(*m_physXSDK, m_physXVisualDebugger);
  ...
  //m_physXCharacterControllerManager = PxCreateControllerManager(*m_physXScene);
...
{ 
  ...
  if (m_physXVisualDebugger)
  {
    m_physXVisualDebugger->release();
    m_physXVisualDebugger = NULL;
  }
...
bool EuphoriaManager::connectToPhysXVisualDebugger()
{
  ...
  if (m_physXVisualDebugger != 0)
  {
    physx::PxPvdTransport* transport = nullptr;
    if (m_physXVisualDebuggerFilename[0] == '\0')
    {
      m_physXVisualDebugger->disconnect();
    ...
    else
    {
      ...
      physx::PxPvdTransport* transport = physx::PxDefaultPvdFileTransportCreate(m_physXVisualDebuggerFilename);
    }


    m_physXVisualDebugger->connect(*transport, physx::PxPvdInstrumentationFlag::eDEBUG |
        physx::PxPvdInstrumentationFlag::eMEMORY |
        physx::PxPvdInstrumentationFlag::ePROFILE);
...
physx::PxScene* EuphoriaManager::createScene(const NMP::Vector3* gravity)
{
  ...
  m_physXScene->setClientBehaviorFlags(0, (physx::PxClientBehaviorFlags)15); // Setting all the basic bits for now, see MORPH-11272
...
void EuphoriaManager::destroyScene()
{
  ...
  NMP_ASSERT(!m_physXScene || m_physXScene->getNbActors(physx::PxActorTypeFlag::eRIGID_STATIC | physx::PxActorTypeFlag::eRIGID_DYNAMIC) == 0);
  ...
  float                       angularDamping)
{
  ...
  if (dynamic)
  {
    ...
    dynamicActor->setClientBehaviorFlags((physx::PxActorClientBehaviorFlags)g_clientBehaviourBits);
  ...
  else
  {
    ...
    staticActor->setClientBehaviorFlags((physx::PxActorClientBehaviorFlags)g_clientBehaviourBits);
