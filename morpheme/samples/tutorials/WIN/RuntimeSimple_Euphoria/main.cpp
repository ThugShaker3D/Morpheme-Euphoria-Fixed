  ...
  Game::CharacterManagerEuphoria& characterManager)
{
  ...
  static const float maxTime = 600.0f;
  ...
  for (frameCounter = 0 ; frameCounter != maxFrames ; ++frameCounter)
  {
    ...
    if (GetAsyncKeyState('W') & 0x8000)
        gameCharacter->GetEuphoriaCharacter()->getBody().getPhysicsRig()->receiveWrenchImpulse(NMP::Vector3(0, 0, 7), NMP::Vector3(0, 0, 0));
    else if(GetAsyncKeyState('S') & 0x8000)
        gameCharacter->GetEuphoriaCharacter()->getBody().getPhysicsRig()->receiveWrenchImpulse(NMP::Vector3(0, 0, -7), NMP::Vector3(0, 0, 0));

    if (GetAsyncKeyState('A') & 0x8000)
        gameCharacter->GetEuphoriaCharacter()->getBody().getPhysicsRig()->receiveWrenchImpulse(NMP::Vector3(7, 0, 0), NMP::Vector3(0, 0, 0));
    else if (GetAsyncKeyState('D') & 0x8000)
        gameCharacter->GetEuphoriaCharacter()->getBody().getPhysicsRig()->receiveWrenchImpulse(NMP::Vector3(-7, 0, 0), NMP::Vector3(0, 0, 0));

    static bool getup_pressed = false;
    static bool euphoria_pressed = false;

    if (GetAsyncKeyState('Q') & 0x8000 && !getup_pressed)
    {
        getup_pressed = true;
    ...
    if (GetAsyncKeyState('E') & 0x8000 && !euphoria_pressed)
    {
        euphoria_pressed = true;
        gameCharacter->broadcastRequestMessage(euphoriaMessageID, true);
    }
    else
        euphoria_pressed = false;
