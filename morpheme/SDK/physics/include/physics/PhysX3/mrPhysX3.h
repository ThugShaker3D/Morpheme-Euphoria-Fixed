...
static inline physx::PxTransform getActorTransform(const physx::PxActor& actor)
{
  if (actor.is<physx::PxRigidActor>())
    return actor.is<physx::PxRigidActor>()->getGlobalPose();
...
{
  const physx::PxRigidBody* body = actor.is<physx::PxRigidBody>();
  if (body)
  ...
  physx::PxFilterData& filterData)
{
  ...
  {
    filterAttr |= physx::PxFilterObjectFlag::eTRIGGER;
  }

  physx::PxActor* actor = shape->getActor();
  NMP_ASSERT(actor);

  physx::PxRigidDynamic* rigidDynamicActor = actor->is<physx::PxRigidDynamic>();
  if (rigidDynamicActor)
  {
    if (!rigidDynamicActor->is<physx::PxArticulationLink>())
    {
      if (rigidDynamicActor->getRigidBodyFlags() & physx::PxRigidBodyFlag::eKINEMATIC)
      {
        filterAttr |= physx::PxFilterObjectFlag::eKINEMATIC;
      }
