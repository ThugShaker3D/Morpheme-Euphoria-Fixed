...
------------------------------------------------------------------------------------------------------------------------
-- Per animation set predictive uneven terrain hips section
------------------------------------------------------------------------------------------------------------------------
  ...
  panel:beginVSizer{ flags = "expand", proportion = 1 }
    ...
    panel:addVSpacer(10)
      ...
      panel:setFlexGridColumnExpandable(2)
        ...
        calculateLeftKneeAxisButton:setOnClick(
          function(self)
            ...
            for i, selectedObject in selection do
              if getType(selectedObject) == "PredictiveUnevenTerrain" then
        ...
        -- Event track ID
        local leftEventTrackIDName = string.format("LeftEventTrackID", legType)
        local leftEventTrackIDText = attributeEditor.addAttributeLabel(panel, "Event Track ID", selection, leftEventTrackIDName)
        local leftEventTrackIDWidget = attributeEditor.addAttributeWidget(panel, leftEventTrackIDName, selection, set)        
    ...
    panel:beginVSizer{ label = "Right", flags = "expand;group", proportion = 1 }
      ...
      panel:setFlexGridColumnExpandable(2)
        ...
        -- Event track ID
        local rightEventTrackIDName = string.format("RightEventTrackID", legType)
        local rightEventTrackIDText = attributeEditor.addAttributeLabel(panel, "Event Track ID", selection, rightEventTrackIDName)
        local rightEventTrackIDWidget = attributeEditor.addAttributeWidget(panel, rightEventTrackIDName, selection, set)
...
------------------------------------------------------------------------------------------------------------------------
-- Per animation set predictive uneven terrain limits section
------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------
-- Per animation set predictive uneven terrain prediction section
------------------------------------------------------------------------------------------------------------------------
local perPredictionSetSection = function(panel, selection, attributes, set)
  -- check if the selection contains any referenced objects
  local hasReference = containsReference(selection)
  ...
  panel:beginVSizer{ flags = "expand", proportion = 1 }
    ...
    panel:setFlexGridColumnExpandable(2)
        ...
        attributeEditor.addAttributeLabel(panel, "Slope Angle Limit", selection, "PredictionSlopeAngleLimit")
        local PredictionSlopeAngleLimitWidget = attributeEditor.addAttributeWidget(panel, "PredictionSlopeAngleLimit", selection, set)

        -- PredictionLateralAngleLimit
        attributeEditor.addAttributeLabel(panel, "Lateral Angle Limit", selection, "PredictionLateralAngleLimit")
        local PredictionLateralAngleLimitWidget = attributeEditor.addAttributeWidget(panel, "PredictionLateralAngleLimit", selection, set)

        -- PredictionCloseFootbaseTolFrac
        attributeEditor.addAttributeLabel(panel, "Close Footbase Tolerance", selection, "PredictionCloseFootbaseTolFrac")
        local PredictionCloseFootbaseTolFracWidget = attributeEditor.addAttributeWidget(panel, "PredictionCloseFootbaseTolFrac", selection, set)        
  ...
  -- enable text boxes based on the value of governing booleans
  ----------------------------------------------------------------------------------------------------------------------
  local syncPredictionInterfaceFromAttributes = function()
    attributeEditor.logEnterFunc("syncPredictionInterfaceFromAttributes")

    PredictionSlopeAngleLimitWidget:enable(not hasReference)
    PredictionLateralAngleLimitWidget:enable(not hasReference)
    PredictionCloseFootbaseTolFracWidget:enable(not hasReference)

    attributeEditor.logExitFunc("syncPredictionInterfaceFromAttributes")
  end

  syncPredictionInterfaceFromAttributes()
end

------------------------------------------------------------------------------------------------------------------------
-- Adds the predictive uneven terrain hips section to the attribute editor
------------------------------------------------------------------------------------------------------------------------
attributeEditor.predictiveUnevenTerrainHipsDisplayInfoSection = function(panel, displayInfo, selection)
  attributeEditor.logEnterFunc("attributeEditor.predictiveUnevenTerrainHipsDisplayInfoSection")
  ...
  local rollup = panel:addRollup{
    ...
    flags = "mainSection",
    name = string.format("PredictiveUnevenTerrainHipsDisplayInfoSection")
  }
  ...
  attributeEditor.logExitFunc("attributeEditor.predictiveUnevenTerrainHipsDisplayInfoSection")
end

------------------------------------------------------------------------------------------------------------------------
-- Adds the predictive uneven terrain leg sections to the attribute editor
------------------------------------------------------------------------------------------------------------------------
attributeEditor.predictiveUnevenTerrainLegDisplayInfoSection = function(panel, displayInfo, selection)
  attributeEditor.logEnterFunc("attributeEditor.predictiveUnevenTerrainLegDisplayInfoSection")
  ...
  local rollup = panel:addRollup{
    label = displayInfo.title,
    flags = "mainSection",
    name = "PredictiveUnevenTerrainLegDisplayInfoSection",
  }
  local rollPanel = rollup:getPanel()
  ...
  attributeEditor.logExitFunc("attributeEditor.predictiveUnevenTerrainLegDisplayInfoSection")
end

------------------------------------------------------------------------------------------------------------------------
-- Adds the predictive uneven terrain limits section to the attribute editor
------------------------------------------------------------------------------------------------------------------------
attributeEditor.predictiveUnevenTerrainLimitsDisplayInfoSection = function(panel, displayInfo, selection)
  attributeEditor.logEnterFunc("attributeEditor.predictiveUnevenTerrainLimitsDisplayInfoSection")
  ...
  local rollup = panel:addRollup{
    label = displayInfo.title,
    flags = "mainSection",
    name = "predictiveUnevenTerrainLimitsDisplayInfoSection",
  }
  local rollPanel = rollup:getPanel()
  ...
  attributeEditor.logExitFunc("attributeEditor.predictiveUnevenTerrainLimitsDisplayInfoSection")
end

------------------------------------------------------------------------------------------------------------------------
-- Adds the predictive uneven terrain prediction section to the attribute editor
------------------------------------------------------------------------------------------------------------------------
attributeEditor.predictiveUnevenTerrainPredictionDisplayInfoSection = function(panel, displayInfo, selection)
  attributeEditor.logEnterFunc("attributeEditor.predictiveUnevenTerrainPredictionDisplayInfoSection")
  ...
  local rollup = panel:addRollup{
    label = displayInfo.title,
    flags = "mainSection",
    name = "predictiveUnevenTerrainPredictionDisplayInfoSection",
  }
  local rollPanel = rollup:getPanel()
  ...
  rollPanel:beginVSizer{ flags = "expand" }
    rollPanel:addAnimationSetWidget{
      attributes = attrs,
      displayFunc = perPredictionSetSection,
      flags = "expand",
      proportion = 1,
    }
  rollPanel:endSizer()

  attributeEditor.logExitFunc("attributeEditor.predictiveUnevenTerrainPredictionDisplayInfoSection")
end
