  ...
  undoBlock(function()
    for _, object in selection do
      ...
      if cpNode ~= nil and (string.find(getType(cpNode), "ControlParameter") ~= nil) then
        setAttribute(object .. ".InitValueX", getDefaultValue(cpNode))
      end
    end
  end)
end
...
------------------------------------------------------------------------------------------------------------------------
-- Adds an OperatorSmoothFloat display info section.
------------------------------------------------------------------------------------------------------------------------
attributeEditor.operatorSmoothFloatDisplayInfoSection = function(rollContainer, displayInfo, selection)
  attributeEditor.logEnterFunc("attributeEditor.operatorSmoothFloatDisplayInfoSection")

  -- first add the ui for the section
  local rollup = rollContainer:addRollup{ label = displayInfo.title, flags = "mainSection", name = "operatorSmoothFloatDisplayInfoSection"  }
  ...
  rollPanel:beginHSizer{ flags = "expand", proportion = 1 }
    ...
    rollPanel:beginFlexGridSizer{ cols = 2, flags = "expand", proportion = 1 }
      ...
      local attribute = "InitValueX"
      local initialLabel = attributeEditor.addAttributeLabel(rollPanel, "Initial value", selection, attribute)
      attributeEditor.addAttributeWidget(rollPanel, attribute, selection, set)
  ...
  attributeEditor.logExitFunc("attributeEditor.dataTypeSmoothFloatDisplayInfoSection")
end
